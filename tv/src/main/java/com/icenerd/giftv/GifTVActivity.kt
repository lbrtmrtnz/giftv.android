...
class GifTVActivity : Activity() {
            ...
            if (intent.action == SIGNAL_CHANGE_CHANNEL) {
                ...
                if (updateStatus == null) {
                    ...
                    if (BuildConfig.DEBUG) {
                        try {
                            Log.d(TAG, "Status: ${updateStatus.getJSONObject()}")
    ...
    override fun onStart() {
        ...
        if (nsName == null || nsName.isEmpty()) {
            ...
            mNetworkService!!.setHandler(ClientMessageHandler(this))
        }
    }
    ...
    private fun gif_rotation() {
        if (mNextPosition >= mGifs.size) {
            ...
            while (i < mNextPosition + BATCH_SIZE / 2 && i < mGifs.size) {
                mGifLoader!![if (model.size < SIZE_LIMIT_BYTES) model.original?:"" else model.downsized?:"", null]
                i++
            }
        }
        mNextPosition++
    ...
    private fun LOAD_gif(container: GifLoader.GifContainer) {
        mGifImageView!!.setImageDrawable(container.gif)
        val drawable = mGifImageView!!.drawable as GifDrawable
